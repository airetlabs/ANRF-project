...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-blue-50 px-6" style={{ overflow: "hidden" }}>
      <div className="w-full max-w-3xl">
        ...
        <div className="anim flex items-center justify-between mb-7 pb-7 border-b border-slate-200">
          <div>
            ...
            <h1 className="text-5xl font-bold text-slate-900 tracking-tight leading-none">AcadAIsist</h1>
        ...
        <div className="anim flex items-center justify-between flex-wrap gap-3 pt-5 border-t border-slate-200">
          ...
          <div className="flex gap-2">
            <button onClick={() => router.push("/login")} className="bg-white hover:bg-slate-50 active:scale-95 text-slate-700 font-medium px-6 py-3 rounded-xl border border-slate-300 transition-all duration-150 text-sm">
              Sign In
            </button>
            <button onClick={() => setShowRoleModal(true)} className="bg-blue-800 hover:bg-blue-900 active:scale-95 text-white font-semibold px-7 py-3 rounded-xl transition-all duration-150 text-sm shadow-md">
              Get Started
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4" onClick={() => setShowRoleModal(false)}>
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              {roles.map((role) => (
                <button key={role.title} onClick={() => router.push(role.route)} className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-150 text-left ${role.color}`}>
            ...
            <button onClick={() => setShowRoleModal(false)} className="w-full mt-4 text-slate-400 text-sm hover:text-slate-600 transition-colors">
              Cancel
            </button>
          </div>
        </div>
