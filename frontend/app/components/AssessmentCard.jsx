...
export default function AssessmentCard({
  ...
  const continueEditing = () => {
    ...
    if (setSelectedYears) {
      const yrs = (assessment.years || []).map(y => ({
        value: y,
        label: y
      }));
  ...
  const deleteAssessment = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://anrf-project-production-a47a.up.railway.app/assessment/delete/${assessment._id}`,
        { method: "DELETE" }
      );
    ...
    <div className="bg-white rounded-[30px] border border-slate-200 p-8 shadow-sm hover:shadow-md transition flex flex-col justify-between gap-6">
        ...
        <div className="space-y-2">
          ...
          {assessment.subjectName && (
            <div className="flex justify-between items-center text-sm">
              ...
              <p className="font-semibold text-slate-800 text-right max-w-[55%] truncate">
                {assessment.subjectName}
              </p>
            </div>
          )}
      ...
      <div className="flex flex-col gap-3 pt-2 border-t border-slate-100">
        ...
        <button
          ...
          className="w-full bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 py-3.5 rounded-xl font-semibold text-sm transition"
        >
          Delete
        </button>
